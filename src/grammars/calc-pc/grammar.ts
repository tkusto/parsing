...
import { Parser, Regex, Seq, Alt } from '../../pcs/combinators';

function Num() {
  const parse = Regex(/\d+(?:\.(\d+))?/);
  const parseNum: Parser = (src, index) => parse(src, index).map(t => t.produce({
    type: 'number',
...
function Id() {
  ...
  const parseId: Parser = (src, index) => parse(src, index).map(t => t.produce({
    type: 'identifier',
    value: t.value[0]
  }));
  return parseId;
...
function Mul() {
  const parseMul: Parser = (src, index) => {
    const parse = Seq([
      Alt(Id(), Num(), Paren()),
      Regex(/\s*([*/%])\s*/),
...
function Add() {
  const parseAdd: Parser = (src, index) => {
    const parse = Seq([
      Alt(Mul(), Id(), Num(), Paren()),
      Regex(/\s*([-+])\s*/),
...
function Assign() {
  const parseAssign: Parser = (src, index) => {
...
function Factor() {
  const parseFactor: Parser = (src, index) => {
...
function Expr() {
  const parseExpr: Parser = (src, index) => {
...
function Paren() {
  const parseParen: Parser = (src, index) => {
    const parse = Seq([
      Regex(/\s*\(\s*/),
      Expr(),
      Regex(/\s*\)\s*/)
